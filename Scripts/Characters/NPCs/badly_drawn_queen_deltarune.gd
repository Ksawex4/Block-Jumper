...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if NovaFunc.GetPlayerFromGroup(body.name) and !interractDelay:
		interractDelay = true
		$Label.text = ""
		if texter < text.size() - 1 and secretQueen:
			texter += 1
		...
		var currentText = text[texter].replace("PLAYER", body.name)
		if currentText == "I am spinning right now":
			spin = true
